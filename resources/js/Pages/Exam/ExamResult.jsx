...
const ExamResult = ({ auth, title, subject, data,mark   }) => {
    // moment.locale('id')
    ...
    // const hours = convertMinsToHrsMins(subject.exam_duration)

    const arrJawaban = data.answer
    let terjawab = 0
    ...
    const banyakSoal = subject.exam.length
    // console.log('data')
    // console.log(data)
    return (
        <Authenticated user={auth.user}>
            ...
            <div className="m-6">
                ...
                <h1 className="mt-4 text-3xl">{title} - <span>{subject.name}</span></h1>
                {/* <h1 className="mt-2 font-bold">Dibuka:
                <span className="font-medium"> {moment(subject.exam_started).format('LLLL')}</span>
                </h1>
                <h1 className="font-bold ">Ditutup: <span className="font-medium"> {moment(subject.exam_ended).format('LLLL')}</span></h1> */}
            ...
            <div className="pb-16 m-6">
                <h1>Hasil TryOut - {auth.user.name}</h1>
                ...
                <h1 className='text-center text-red-950'>Silahkan kembali ke homepage untuk mencoba lagi</h1>
